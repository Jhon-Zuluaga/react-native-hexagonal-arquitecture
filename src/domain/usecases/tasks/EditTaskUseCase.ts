...
export class EditTaskUseCase {
    ...
    async execute (taskId: string, newTitle: string, currentTasks: Task[]): Promise<EditTaskResult>{
        const task = currentTasks.find(t => t.id === taskId);
        if(!task) return {success: false, error: 'TASK_NOT_FOUND'};

        if(!isValidTaskTitle(newTitle)) return { success: false, error: 'INVALID_TITLE'};
